...
class GANimationModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.net_gen = model_utils.define_G(self.opt.img_nc, self.opt.aus_nc, self.opt.ngf, use_dropout=self.opt.use_dropout,
                                                 init_gain=self.opt.init_gain, gpu_ids=self.gpu_ids)
        self.models_name.append('gen')

        if self.is_train:
            self.net_dis = model_utils.define_D(self.opt.img_nc, self.opt.aus_nc, self.opt.final_size, self.opt.ndf,
                                                     init_gain=self.opt.init_gain, gpu_ids=self.gpu_ids)
